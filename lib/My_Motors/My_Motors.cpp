...
#if ARDUINO >= 100		//Defined in the c_cpp_properties.json
  #include "Arduino.h"
#else
  #include "WProgram.h"
#endif
...
void My_Motors::getRPM(long ticks, String units){  
  ...
  if (dt!=0){ // To prevent Nan and inf values
	...
	inst_rpm = (0.25 * 1000.0 * dn / dt)*(60.0 / ppr); // Calculate instantaneous RPM. The 0.25 is used because a quadrature encoder gives 4 ticks the actual RPM
...
void My_Motors::set_Ninit(int ticks){n_init = ticks;}
int My_Motors::get_Dn(int enc_count){return int(0.25*(enc_count - n_init));}